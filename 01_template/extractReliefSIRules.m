function [labels, resSim] = extractReliefSIRules( V, E, templ, FV)
...
resSim = simMeasureSITemplate( V, templ.T, templ.params, @NLC , pd);
% resSim : SI similarity measures (NLC) w.r.t. three classes;
%          relief, relief boundary, background

nV = size(resSim, 1);
candidates = resSim(:,1) > resSim(:,3) * 1.1; %1.1
cand_grps = conComp( E, candidates );

accurateCandidates = resSim(:,1) > 0.70; %0.85
