function [FaceSFM,VertexSFM,wfp] = CalcCurvature(FV,VN,FN,Varea,Vcorner,up,vp)
%% Summary
...
%VN - nX3 matrix (n = number of vertices) containing the normal at each vertex
%FN - mX3 matrix (m  =  number of faces) containing the normal of each face
%OUTPUT:
...
for i = 1:size(FV.faces,1)
    ...
    wfp(i,1) = Vcorner(i,1)/Varea(FV.faces(i,1));
    wfp(i,2) = Vcorner(i,2)/Varea(FV.faces(i,2));
    wfp(i,3) = Vcorner(i,3)/Varea(FV.faces(i,3));
